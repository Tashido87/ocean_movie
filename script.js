 ...
 * Updated: Static JSON + Fuse.js + Genre Normalization (Animated -> Animation)
 */
...
const CONFIG = {
    // These keys are no longer used for fetching in the frontend (since we use JSON), 
    // but kept here to preserve existing structure.
...
// --- NEW: Genre Mapping (Fixes typos or duplicates) ---
const GENRE_FIXES = {
    'animated': 'Animation',
    'cartoon': 'Animation',
...
// =========================================
// 2. DATA FETCHING (STATIC JSON & PARSING)
// =========================================
...
// Helper: Parse Row (Includes Genre Normalization)
function parseRow(row, type) {
    ...
    return {
        ...
        // --- GENRE NORMALIZATION APPLIED HERE ---
        genre: row[3] ? row[3].split(',').map(g => {
            let clean = g.trim();
            let lower = clean.toLowerCase();
            // If the genre is in our fix list, use the fixed version
...
async function loadAllContent() {
    try {
        ...
        if(DOM.loadingScreen) DOM.loadingScreen.innerHTML = '<p>Error loading data. Try refreshing.</p>';
    }
}
...
// Updated: Includes Genre Normalization for Config Filters
function getItemsFromConfig(config) {
    ...
    // Filter by Genre (UPDATED LOGIC)
    if (config.genre) {
        let genreTarget = config.genre.toLowerCase();
        // If config sheet says "Animated", convert to "Animation" to match data
...
// =========================================
// 5. MAIN LISTING VIEW UPDATE (FUSE.JS + FILTERS)
// =========================================
