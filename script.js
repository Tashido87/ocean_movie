...
// --- GENRE MAPPING (Fixes Typos/Duplicates) ---
const GENRE_FIXES = {
    // Sci-Fi variations
...
// Helper: Parse Row (With Smart Casing & Normalization)
...
function navigateTo(pageName, data = null) {
    ...
    if (pageName === 'home') {
        ...
        DOM.listingView.classList.remove('hidden');
        state.listing.baseTitle = pageName === 'search-results' ? 'Search Results' : 'Browse';
        ...
        DOM.listingView.classList.remove('hidden');
        state.listing.baseTitle = data.title;
        populateFilterOptions(null); 
        state.listing.currentPage = 1;
